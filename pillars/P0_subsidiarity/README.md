# Pillar 0 (P0) — Subsidiarity, Consent & Exit
...
**Scope:** Foundational layer beneath every operational pillar. P0 defines *who consents to be governed by this protocol, at what scale, on what matters, and how they withdraw.*

---

## The Problem P0 Resolves

Every pillar in the prior protocol operated under an implicit assumption: that the continental federation is the default scale of decision, and that participation is a standing condition rather than a renewable act. That assumption had three costs:
...
P0 closes all three gaps at the architectural layer.
...
## What P0 Does NOT Touch

P0 is a scale-and-consent layer. It does **not**:
...
- Create new central authority. Every P0 rule reduces or constrains the federation's default reach; no rule extends it.
...
## How P0 Interacts with the Rest of the Protocol

- **Master Protocol §0** — P0 is added as the foundational layer beneath the twelve pillars. The §0 invariants (survival unconditional, CSM floor, separation of concerns, etc.) are federation-scale and not subject to unit-level modification or unit-level exit.
- **Pillars 1–12** — each pillar is read through P0: its default scale is the smallest competent unit; its exceptions-to-smallest-scale are specified with subsidiarity rationale.
- **Founding commitments** — FC-120, FC-121, FC-122 are the numerical anchors for P0. They are Tier 1 protected per `/architecture/invariants.md` (Category D — Governance & Exit).
- **Architecture layer** — P0 integrates with the drift chain: a unit-exit event is drift-chain-logged (new row, federation scope reduced, signatures of the exiting unit's representatives and 7 of 9 Tier 1 holders acknowledging). An exit cannot proceed silently.
...
Extension — adding finer-grained scales (e.g., sub-neighborhood clusters for specific decision classes), adding consent mechanisms, or strengthening exit rights — may proceed by Tier 2 amendment. **Weakening any P0 right requires Tier 1 amendment with the full H-3 Refounding Convention.**

---

*P0 is the answer to "does this protocol respect the people it governs?" The answer is: only to the extent that they can decline it, customize it at their own scale, and leave it at bounded cost. P0 makes all three operative.*
